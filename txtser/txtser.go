...
// Package txtser provides types to read and write textual archives from the C++ Boost Serialization library.
//
// Writing values to an output textual archive can be done like so:
//
//	enc := txtser.NewEncoder(w)
//	err := enc.Encode("hello")
//
// And reading values from an input textual archive:
//
//	dec := txtser.NewDecoder(r)
//	str := ""
//	err := dec.Decode(&str)
//
// For more informations, look at the examples for Encoder, Decoder and read/write Buffer.
package txtser // import "github.com/go-boostio/boostio/txtser"

//go:generate go run ./testdata/gen-textual-archive.go
...
var (
	ErrNotBoost         = errors.New("txtser: not a Boost textual archive")
	ErrInvalidHeader    = errors.New("txtser: invalid Boost textual archive header")
...
func (a Arch) Header() Header {
	...
	case Arch64:
		return bser64Hdr
	default:
		panic(fmt.Errorf("txtser: invalid architecture (size %d)", int(a)))
	}
}
...
// Unmarshaler is the interface implemented by types that can unmarshal a textual Boost description of themselves.
type Unmarshaler interface {
	UnmarshalBoost(r *RBuffer) error
}

// Marshaler is the interface implemented by types that can marshal themselves into a valid textual Boost serialization archive.
type Marshaler interface {
	MarshalBoost(w *WBuffer) error
}

// Header describes a textual boost archive.
type Header struct {
	Version uint16
	Flags   uint64
}
...
// TypeDescr describes an on-disk textual boost archive type.
