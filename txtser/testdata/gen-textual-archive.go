...
func build(arch int) error {
	...
	tmp, err := os.MkdirTemp("", "boostio-txtser-write-")
	if err != nil {
	...
	cmd := exec.Command("c++", "-m"+strconv.Itoa(arch), "-o", "bwrite", "write.cxx", "-lboost_serialization")
	cmd.Dir = tmp
	...
	err = os.WriteFile(fmt.Sprintf("testdata/data%d.txt", arch), archive.Bytes(), 0644)
	if err != nil {
		return fmt.Errorf("could not save textual archive: %w", err)
	}
